...
if($action == 'allocatestudent')
{
	if(isset($_POST['allocatestudent']))
	{
		$data = $_POST['data'];
		$charges = get_single_row('transport_pickup_points', array('tr_place_id' => $data['pickup_point_id']));
		$data['payble_charges'] = $charges['annual_charges'];
		
		
		$data['received_amount'] = 0;
		$data['concession'] = 0;
		$data['actual_received'] = 0;
		$data['ledger_id'] = $charges['ledger_id'];

		insert_into('transport_student_allocation', $data);

		header('Location: ?pid=75&action=allottedstudent&ac_id='.$data['acdemic_year_id'].'&class_id='.$_POST['class'].'&division_id='.$_POST['division']);
		exit;
	}
}
if($action == 'delete_allottedstudent')
{
	
	delete_where('transport_student_allocation', array('transport_student_allocation_id' => $_GET['transport_student_allocation_id']));

	header('Location: ?pid=75&action=allottedstudent&ac_id='.$_GET['ac_id'].'&class_id='.$_GET['class_id'].'&division_id='.$_GET['division_id']);
}
?>
