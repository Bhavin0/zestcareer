...
function classname($cid){
 ...
 function bookname($bid){
	$book = "SELECT `lbook_title` FROM `es_libbook` WHERE `es_libbookid`=" . $bid ;
    $bookName = getarrayassoc($book);
	return $bookName['lbook_title'];
 }
 
 function hostelitemname($bid){
	 $item = "SELECT `in_item_name` FROM `es_in_item_master` WHERE `es_in_item_masterid`=" . $bid ;
 ...
 function deptname($sid){
  ...
  function postname($sid1){
	 ...
	 $post = "SELECT `es_postname` FROM `es_deptposts` WHERE `es_deptpostsid`='" . $sid1 . "'";
 ...
 function studentname($studentid){
  ...
  function libcategoryname($sid){
	$sel_category = "SELECT  `lb_categoryname` FROM `es_categorylibrary` WHERE `es_categorylibraryid`='" . $sid . "'";
    $categotyName = getarrayassoc($sel_category);
	return  $categotyName['lb_categoryname'];
 }
  function libsubctegoryname($sid){
	$sel_subcat = "SELECT  `subcat_scatname` FROM `es_subcategory` WHERE `es_subcategoryid`='" . $sid . "'";
...
*/  function schoolfees($from_finance,$to_finance){
	 $sel_schoolgrandfee = "SELECT SUM(`feeamount`) AS SUMAMT FROM `es_feepaid` WHERE fi_fromdate='" . $from_finance . "' 
...
{	
	$get_school = "SELECT * FROM `es_schools` where `school_id`='" . $schoolid . "'";
...
function get_StudAttend($at_std_class){
	$classes_data = array();
	  $sel_classes    = "SELECT * FROM `es_preadmission` WHERE `pre_class`= '$at_std_class' AND status!= 'inactive' AND status!= 'resultawaiting' 
	                AND pre_fromdate = '".$_SESSION['eschools']['from_acad']."'  AND pre_todate= '".$_SESSION['eschools']['to_acad']."'";
	$rs_classes     = mysql_query($sel_classes); 	
	if (mysql_num_rows($rs_classes ) >0 ){
		while ($classes_det = mysql_fetch_assoc($rs_classes)){
			$classes_data[] = $classes_det;
...
function get_StudAttend_report($at_std_class){
	$classes_data = array();
	$sel_classes    = "SELECT DISTINCT `at_reg_no` ,`at_stud_name` FROM `es_attend_student` WHERE  `at_attendance_date` BETWEEN  '$from_acad' AND  '$to_acad' AND `at_std_class`= '$at_std_class'";
	return getamultiassoc($sel_classes);
...
function getstudentdetails($at_staff_dept) {
	$classes_data = array();
	$sel_classes    = "SELECT * FROM `es_staff` WHERE `st_department`= '$at_staff_dept' AND status='added' AND selstatus='accepted' AND tcstatus='notissued' ORDER BY `st_firstname`";
	$rs_classes     = mysql_query($sel_classes); 
...
function exam_papercode($id){
	$exam = "SELECT paper_code FROM es_exambuilder WHERE exam_id=" . $id;
...
$dt1 = substr($birthdate,0,2);
 ...
 function showstaff($es_class,$perid){
 $staff_arr = array();
  $t1_staff_sql = "SELECT `es_staffid`,`st_firstname`,`st_lastname` FROM `es_staff` ST WHERE ST.teach_nonteach='teaching' AND ST.status='added' AND ST.selstatus='accepted' AND ST.tcstatus='notissued' AND ST.es_staffid NOT IN(SELECT ".$perid." FROM es_timetable_staff TS WHERE TS.es_class!='".$es_class."' ) ";
					...
					$staff_arr[] = "--Staff--";
						while ($data = @mysql_fetch_assoc($rs_qury)){
							 ...
							 $staff_arr[$data['es_staffid']] = strtolower($data['st_firstname']);
						}
						return $staff_arr;
					}
 return null;
 }
 function showsubjects($es_class){
 $staff_arr = array();
...
WHERE classid =".$es_class."
		...
		//$sel_subject = "SELECT * FROM `es_subject` WHERE `es_subjectshortname` ='$sub_class'";
						 ...
						 $rs_qury   = @mysql_query($sel_subject);
					if (@mysql_num_rows($rs_qury)>0){
					$subjects_arr[] = "--Subject--";
						while ($data = @mysql_fetch_assoc($rs_qury)){
							
							 $subjects_arr[$data['es_subjectid']] = strtolower($data['es_subjectname']);
						}
						return $subjects_arr;
					}
 return null;
