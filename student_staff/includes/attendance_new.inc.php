...
/*if (!isset($_SESSION['eschools']['admin_user']) || $_SESSION['eschools']['admin_user']=="" ) {
	...
	if($_GET['action']=='stud_attend' && (isset($_POST['submit']) || isset($_POST['submit_and_send_sms'])))
	{
		...
		for($i=0; $i<count($_POST['student_id']);$i++)
		{
			...
			if(isset($_POST['submit_and_send_sms']) && $_POST['attendance'][$i]=='A')
			{
				$student = mysqli_fetch_array(mysqli_query($mysqli_con, "SELECT pre_name, middle_name, pre_lastname, pre_sms_no FROM es_preadmission WHERE es_preadmissionid=".$_POST['student_id'][$i]));

				$msg  = strtoupper("THIS IS JUST CONFIRMATION MESSAGE FROM ".$section_name[0].", THAT YOUR SON/DAUGTHER ".$student['pre_name']." ".$student['middle_name']." ".$student['pre_lastname']." IS ABSENT TODAY.");

				if(preg_match("/^[6-9][0-9]{9}$/", $student['pre_sms_no']))
				{
					$url = "http://bhashsms.com/api/sendmsg.php?user=hardikmanglani&pass=reset123&sender=GSCSMS&phone=".$student['pre_sms_no']."&text=".urlencode($msg)."&priority=sdnd&stype=normal";
		...
		if(isset($_POST['submit_and_send_sms']))
		{
			echo"PLEASE WAIT WHILE WE ARE SENDING SMS...";
		}
		?>
		<script type="text/javascript">
			$(document).ajaxStop(function(){
    			window.location.href = "?pid=55&action=stud_attend_report&attendance_id=<?php echo $attendance_id; ?>";
    			document.write('<a href="?pid=55&action=stud_attend_report&attendance_id=<?php echo $attendance_id; ?>">CLICK HERE TO VIEW ATTENDANCESHEET</a>')
			});
		</script>
		<?php
		header('Location: ?pid=55&action=stud_attend_report&attendance_id='.$attendance_id);
		exit;
	}
	...
	if($_GET['action']=='attendancesheet_edit' && (isset($_POST['submit']) || isset($_POST['submit_and_send_sms'])))
	{
		...
		<script type="text/javascript">
			$(document).ajaxStop(function(){
    			window.location.href = "?pid=55&action=attendancesheets";
			});
		</script>
		<?php
		header('Location: ?pid=55&action=attendancesheets');
		exit;
	}
	...
	if(isset($stud_attend_Submit) && $stud_attend_Submit != "") {
		...
		if(count($errormessage)==0) {		 
		   ...
		   if (isset($differencedate_val) && $differencedate_val > 0 ) {
			   $emsg = 38;
			   header("location:?pid=$pid&action=stud_attend&at_std_class=$at_std_class&emsg=22");
			   exit;
			}
