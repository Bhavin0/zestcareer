	...
	if($_GET['action']=='bonus_teacher')
	{
		// echo $_GET['s_date'];exit;
		 ...
		 <div class="toggle transparent toggle-accordion">
			...
			<input type="hidden" value="<?php echo $st_bonus;?>" name="actual_bonus">
		...
		$i=1;
			...
			?>
				<tr>
					...
					<td><a <?php if($row['status'] == "Pending"){ echo 'class="btn btn-primary"';}else{echo 'class="btn btn-success"';}?> href="query_1.php?action=status_survey&s_status=<?php echo $row['status'];?>&survey_id=<?php echo $row['survey_id']; ?>"><?php echo $row['status'];?></a></td>
				</tr>
			<?php
			}
			?>
