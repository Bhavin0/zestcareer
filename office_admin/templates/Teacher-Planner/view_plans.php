...
<html lang="en-US">
    <head>
        ...
        <title>VIEW STAFF PLANNER</title>
    ...
    <body>
        ...
        ?>
            ...
            <?php
                ...
                $plans = mysqli_query($mysqli_con, "SELECT `teacher_planner`.*, `es_finance_master`.`fi_ac_startdate`, `es_finance_master`.`fi_ac_enddate`, `es_staff`.`st_firstname`, `es_staff`.`st_lastname`, `es_classes`.`es_classname`, `isd_class_division`.`division_name`, `es_subject`.`es_subjectname` FROM `teacher_planner` INNER JOIN `es_finance_master` ON `es_finance_master`.`es_finance_masterid` = `teacher_planner`.`academic_year_id` INNER JOIN `es_staff` ON `es_staff`.`es_staffid` = `teacher_planner`.`teacher_id` INNER JOIN `es_classes` ON `es_classes`.`es_classesid` = `teacher_planner`.`class_id` INNER JOIN `isd_class_division` ON `isd_class_division`.`class_division_id` = `teacher_planner`.`division_id` INNER JOIN `es_subject` ON `es_subject`.`es_subjectid` = `teacher_planner`.`subject_id` ORDER BY academic_year_id DESC, teacher_plannerid ASC") or die(MYSQLI_ERROR($mysqli_con));
                    ...
                    <div id="panel-1" class="panel panel-primary">
                        <div class="panel-heading">
                            <span class="title elipsis">
                                <strong>Planner</strong> <!-- panel title -->
                            </span>
                        ...
                        <div class="panel-body">
                            ...
                            <label class="pull-right">
                                <a href="?pid=141&action=add_plan" class="btn btn-primary">
                                ...
                                <?php $i = 1; while($plan = mysqli_fetch_assoc($plans)) { ?>
                                    <tr>
                                       ...
                                       <td><?php echo $plan['planner_remarks']; ?></td>
                                        ...
                                        <td>
                                            <a href="?pid=141&action=view_plan_detail&plan_id=<?php echo $plan['teacher_plannerid']; ?>" class="btn btn-info btn-xs">&nbsp;<i class="fa fa-eye"></i></a>
                                            <a href="?pid=141&action=edit_plan&plan_id=<?php echo $plan['teacher_plannerid']; ?>" class="btn btn-warning btn-xs">&nbsp;<i class="fa fa-pencil-square-o"></i></a>
                                            <a href="?pid=141&action=delete_plan&plan_id=<?php echo $plan['teacher_plannerid']; ?>" onclick="return confirm('are you sure? you want to delete it?')" class="btn btn-danger btn-xs">&nbsp;<i class="fa fa-trash-o"></i></a>
                                        </td>
                                    </tr>
                                <?php } ?>
                                </tbody>
