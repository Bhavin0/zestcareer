...
class es_attend_staff extends POG_Base
{
	...
	public $pog_attribute_type = array(
		...
		"at_staff_date" => array('db_attributes' => array("TEXT", "DATE")),
		...
		"at_time_in" => array('db_attributes' => array("TEXT", "TIME")),
		"at_time_out" => array('db_attributes' => array("TEXT", "TIME")),
		);
	public $pog_query;
	...
	*/
	...
	function es_attend_staff($at_staff_dept='', $at_staff_date='', $at_staff_id='', $at_staff_name='', $at_staff_desig='', $at_staff_attend='', $at_staff_remarks='', $at_in_time='', $at_out_time='')
	{
		...
		$this->at_in_time = $at_in_time;
		$this->at_out_time = $at_out_time;
	}
	...
	*/
	function Get($es_attend_staffId)
	{
		...
		while ($row = Database::Read($cursor))
		{
			...
			$this->at_in_time = $this->Unescape($row['at_in_time']);
			$this->at_out_time = $this->Unescape($row['at_out_time']);
		}
		return $this;
	}
	...
	*/
	function GetList($fcv_array = array(), $sortBy='', $ascending=true, $limit='')
	{
		...
		while ($row = Database::Read($cursor))
		{
			...
			$es_in_time->at_in_time = $this->Unescape($row['at_in_time']);
			$es_out_time->at_out_time = $this->Unescape($row['at_out_time']);
	...
	*/
	function Save()
	{
		...
		if ($rows > 0)
		{
			...
			`at_staff_remarks`='".$this->Escape($this->at_staff_remarks)."', 
			`at_in_time`='".$this->Escape($this->at_in_time)."', 
			`at_out_time`='".$this->Escape($this->at_out_time)."' where `es_attend_staffid`='".$this->es_attend_staffId."'";
		}
		else
		{
			$this->pog_query = "insert into `es_attend_staff` (`at_staff_dept`, `at_staff_date`, `at_staff_id`, `at_staff_name`, `at_staff_desig`, `at_staff_attend`, `at_staff_remarks`, `at_time_in`, `at_time_out`) values (
			...
			'".$this->Escape($this->at_time_in)."', 
			'".$this->Escape($this->at_time_out)."' )";
		}
