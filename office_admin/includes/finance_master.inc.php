...
if ($action=='school_details') {
	...
	if ($Submit=='Submit')	{
		...
		if($fi_schoolname==""){$errormessage[4]="Enter School / College Name";	}
		if($fi_address==""){$errormessage[5]="Enter Address";	}

		if($fi_email==""){$errormessage[6]="Enter Email";	}
		if($fi_phoneno==""){$errormessage[7]="Enter Phone Number";	}
		if($school_records==0){
		if($fid==""){
		...
		if (count($errormessage)==0){
			...
			if (is_uploaded_file($_FILES['fi_school_logo']['tmp_name'])) {
				...
				$pre_image = $new_thumbname;
					$obj_accounts->fi_school_logo = $pre_image ;
			} else {
								$pre_image = $oldlogoimage;
				$obj_accounts->fi_school_logo = $oldlogoimage;
			}
...
//get list of voucher
	...
	$log_insert_exe=mysql_query($log_insert_sql);
			...
			$emsg = 2;
			header('location:index.php?pid=22&action=voucher&emsg='.$emsg);
			exit;
		}
	$obj_voucher = new es_voucher();
		...
		if (isset($back)){
		header('location:index.php?pid=22&action=voucher');
		exit;
	     }
...
/* Updation of Groups */
  ...
  if($action=='semesters')
  {
  	...
  	{
  		mysql_query("INSERT INTO new_semesters(`academic_year_id`, department_id, from_date, to_date, name) VALUES ('".$academic_year_id."','".$department_id."','".$from_date."','".$to_date."','".$name."')");
  		header('location: ?pid=22&action=semesters');	
  	}
  }
