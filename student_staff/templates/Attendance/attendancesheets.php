                        ...
                        <div class="panel-body">
                                ...
                                <div id="myModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <form action="?pid=140&action=attendancesheets" method="get">
                                                <input type="hidden" value="55" name="pid">
                                                <input type="hidden" name="action" value="attendancesheets">
                            ...
                            <div class="col-md-12 table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <?php
                                        $query = "SELECT `student_attendance`.*, `es_classes`.`es_classname`, CONCAT(`es_staff`.`st_firstname`,' ',`es_staff`.`st_lastname`) AS teacher_name, `isd_class_division`.`division_name` FROM `student_attendance` INNER JOIN `es_classes` ON `es_classes`.es_classesid = `student_attendance`.`standard_id` INNER JOIN `es_staff` ON `es_staff`.`es_staffid` = `student_attendance`.`teacher_id` INNER JOIN `isd_class_division` ON `isd_class_division`.`class_division_id` = `student_attendance`.`division_id` WHERE `student_attendance`.teacher_id = ".$_SESSION['eschools']['user_id'];
                                        if(!isset($_GET['from_date']) && !isset($_GET['to_date']))
                                        {
                                            $query .= " AND student_attendance.attendance_date >= '".date('Y-m-d')."' AND student_attendance.attendance_date <= '".date('Y-m-d')."'";
                                        }
                                        else
                                        {
                                            $query .= " AND student_attendance.attendance_date >= '".$_GET['from_date']."' AND student_attendance.attendance_date <= '".$_GET['to_date']."'";
                                        }
                                        ...
                                        <tr>
                                            ...
                                            <td align="center">
                                                <a href="?pid=55&action=stud_attend_report&attendance_id=<?php echo $test['attendance_id']; ?>" class="btn btn-info btn-xs">&nbsp;<i class="fa fa-table"></i></a>

                                                <a href="?pid=55&action=attendancesheet_edit&attendance_id=<?php echo $test['attendance_id']; ?>" class="btn btn-warning btn-xs">&nbsp;<i class="fa fa-pencil-square-o"></i></a>
                                            </td>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
