					...
					<div id="panel-1" class="panel panel-primary">
						<div class="panel-heading">
							<span class="title elipsis">
								<strong>Assignments</strong> <!-- panel title -->
							</span>
						...
						<div class="panel-body">
							...
							<table class="fooTableInit"><!-- add .fooMinimal for minimal skin and remove foo css theme (footable.standalone.css) -->
								...
								<tbody>
								<?php $i = 1; while($assignment = mysqli_fetch_assoc($assignments)) { ?>
									<tr>
										<td><?php echo $i++; ?></td>
										<td class="foo-cell"> <?php echo YMDtoDMY($assignment['as_createdon']); ?> </td>
										<td><?php echo $assignment['es_classname']; ?>- <?php echo $assignment['division_name']; ?></td>
										<td><?php echo $assignment['es_subjectname']; ?></td>
										<td><?php echo $assignment['as_name']; ?></td>
										<td><?php echo $assignment['person_created']; ?></td>
										<td>
											<a href="?pid=21&action=view_assignment_detail&assignment_id=<?php echo $assignment['es_assignmentid']; ?>" class="btn btn-info btn-xs">&nbsp;<i class="fa fa-eye"></i></a>
											<a href="?pid=21&action=edit_assignment&assignment_id=<?php echo $assignment['es_assignmentid']; ?>" class="btn btn-warning btn-xs">&nbsp;<i class="fa fa-pencil-square-o"></i></a>
											<a href="?pid=21&action=delete_assignment&assignment_id=<?php echo $assignment['es_assignmentid']; ?>" onclick="return confirm('are you sure? you want to delete it?')" class="btn btn-danger btn-xs">&nbsp;<i class="fa fa-trash-o"></i></a>
										</td>
									</tr>
								<?php } ?>
								</tbody>
