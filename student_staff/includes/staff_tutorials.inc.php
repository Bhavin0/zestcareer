...
if(isset($_POST['add_material']))
{
	...
	$material_id = insert_into('es_studymaterial', $data);

	if(isset($_FILES['attachment']['name']) && $_FILES['attachment']['name'] != '')
	{
		$name = $_FILES["attachment"]["name"];
		$ext = end((explode(".", $name)));
		$destination = "../uploads/study_materials/".$material_id.".".$ext;
		move_uploaded_file($_FILES["attachment"]["tmp_name"], $destination);

		update_where('es_studymaterial', array('sm_attachment' => $ext), array('es_studymaterialid' => $material_id));
	}


	header('location: ?pid=36&action=view_material');
	exit;
}
...
if($_GET['action'] == 'view_material')
{
	...
	$query = "SELECT `es_studymaterial`.*, `es_classes`.`es_classname`, `es_subject`.`es_subjectname`, CONCAT(`es_staff`.`st_firstname`,' ',`es_staff`.`st_lastname`) AS `person_created` FROM `es_studymaterial` INNER JOIN `es_classes` ON `es_classes`.`es_classesid` = `es_studymaterial`.`sm_class_id` INNER JOIN `es_subject` ON `es_subject`.`es_subjectid` = `es_studymaterial`.`sm_subject_id` INNER JOIN `es_staff` ON `es_staff`.`es_staffid` = `es_studymaterial`.`created_by` WHERE `es_studymaterial`.`status`='active' AND `es_studymaterial`.`person_type` = 'teacher' AND `es_studymaterial`.`created_by`='".$_SESSION['eschools']['user_id']."' ORDER BY `es_studymaterialid` DESC";
...
{
	$data = $_POST['data'];
	update_where('es_studymaterial', $data, array('es_studymaterialid' => $_GET['studymaterialid'], 'created_by' => $_SESSION['eschools']['user_id']));
...
{
	update_where('es_studymaterial', array('status' => 'deleted'), array('es_studymaterialid' => $_GET['studymaterialid'], 'created_by' => $_SESSION['eschools']['user_id']));
	header('location: ?pid=36&action=view_material');
	exit;
}
