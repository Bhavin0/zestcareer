...
<div id="panel-1" class="panel panel-default">
	...
	<div class="panel-body table-responsive">
		<table class="table">
			...
			<tbody>
		<?php
			$sql = mysql_query("SELECT new_survey.*, es_classes.es_classname, es_subject.es_subjectname, es_staff.st_firstname, es_staff.st_lastname, `isd_class_division`.`division_name` FROM new_survey INNER JOIN es_classes ON es_classes.es_classesid = new_survey.survey_standard INNER JOIN es_subject ON es_subject.es_subjectid = new_survey.survey_subject INNER JOIN es_staff ON es_staff.es_staffid = new_survey.teacher_id INNER JOIN `isd_class_division` ON `isd_class_division`.`class_division_id` = `new_survey`.`survey_division` WHERE new_survey.survey_reviewer = ".$_SESSION['eschools']['user_id']." AND new_survey.survey_type = 1");
			$i = 1;
...
<div id="panel-1" class="panel panel-default">
	...
	<div class="panel-body table-responsive">
		<table class="table">
			...
			<tbody>
		<?php
			$sql = mysql_query("SELECT new_survey.*, es_classes.es_classname, es_subject.es_subjectname, es_staff.st_firstname, es_staff.st_lastname, `isd_class_division`.`division_name` FROM new_survey INNER JOIN es_classes ON es_classes.es_classesid = new_survey.survey_standard INNER JOIN es_subject ON es_subject.es_subjectid = new_survey.survey_subject INNER JOIN es_staff ON es_staff.es_staffid = new_survey.teacher_id INNER JOIN `isd_class_division` ON `isd_class_division`.`class_division_id` = `new_survey`.`survey_division` WHERE new_survey.survey_reviewer = ".$_SESSION['eschools']['user_id']." AND new_survey.survey_type = 2");
			$i = 1;
			while ($row = mysql_fetch_assoc($sql)) {
			?>
				<tr>
...
// print_r($sql);
	...
	<div class="panel-body">
		...
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 form-group">
			...
			<?php
				$sql_division = mysql_query("SELECT * FROM isd_class_division WHERE class_id=".$sql['survey_standard']);
				while($row = mysql_fetch_assoc($sql_division))
				{
					$selected = ($sql['survey_division'] == $row['class_division_id'])?'selected':'';
					echo"<option value='".$row['class_division_id']."'".$selected."> ".$row['division_name']."</option>";
				}
			?>
			</select>
		</div>
