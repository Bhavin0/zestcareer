...
if($_FILES["photo_upload"]["name"] == ''){
   ...
   //	 $st_doj1 = func_date_conversion("d/m/Y","Y-m-d",$st_doj2);
	...
	$sql = mysql_fetch_assoc(mysql_query("SELECT * FROM es_staff WHERE es_staffid =".$sid));
	if($sql['st_department']!=$dept)
	{
		mysql_query("DELETE FROM new_allowencemaster_childs WHERE es_staffid =".$sid);
		mysql_query("DELETE FROM new_deductionmaster_childs WHERE es_staffid =".$sid);
		$allowances = mysql_query("SELECT * FROM es_allowencemaster WHERE alw_dept =".$dept);
		while($row = mysql_fetch_assoc($allowances))
		{
			mysql_query("INSERT INTO new_allowencemaster_childs(es_allowencemasterid, es_staffid, alw_amount, alw_amt_type) VALUES('".$row['es_allowencemasterid']."','".$sid."','0','Amount')");
		}

		$deductions = mysql_query("SELECT * FROM es_deductionmaste WHERE alw_dept =".$dept);
		while($row = mysql_fetch_assoc($deductions))
		{
			mysql_query("INSERT INTO new_deductionmaster_childs(es_deductionmasterid, es_staffid, ded_amount, ded_amt_type) VALUES('".$row['es_deductionmasterid']."','".$sid."','0','Amount')");
		}
	}
