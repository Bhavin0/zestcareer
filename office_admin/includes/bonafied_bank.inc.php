<?php
sm_registerglobal('pid','action','update','emsg','start','submit_from','redirecturl','sno1','date','student_name','father_name','class_name','section','exam_name','exam_date','rollnumber','marks_obtained','rank','dob','charector','status','created_on','conduct','id','update','es_preadmissionid','es_classesid');
/**
	...
	// if($class_name==''){ $errormessage['class_name'] = "Enter Class name";}
		...
		if($dob==''){ $errormessage['dob'] = "Enter Student Date Of Birth";}
			...
			if(count($errormessage)==0){
				 ...
				 $query="update es_bonafiedbank set father_name ='".$father_name ."',section  ='".$section."',exam_date='".$exam_date."',roll_number='".$rollnumber."',marks_obtained='".$marks_obtained."',rank='".$rank."',dob='".$dob."',charector='".$charector."',conduct='".$conduct."',exam_name='".$exam_name."' where id=".$id;
