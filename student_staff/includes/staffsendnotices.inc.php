...
header("location: ?pid=31&action=mailtostaff&st_department=".$staff_detals['st_department']."&es_deptpostsid=".$staff_detals['st_post']."&es_staffid=".$staff_detals['es_staffid']."&type=reply");
exit;
 }
...
header("location: ?pid=31&action=mailtostudents&es_classesid=".$student_detals['pre_class']."&es_preadmissionid=".$student_detals['es_preadmissionid']."&type=reply");
exit;
 }
...
header("location: ?pid=31&action=replyto&es_messagesid=".$es_messagesid."&type=reply");
exit;
 }
	...
	if(isset($submit_staff) && $submit_staff!=''){
		...
		if(isset($id)){
		 ...
		 $db->update('es_notice_messages', "replay_status ='replied'",'es_messagesid =' . $es_messagesid);
					...
					header("location:?pid=31&action=mails_received&emsg=28");	
					exit;	
					}			
...
if($action=="mails_received" || $action=='print_not_list'){
	...
	if(isset($search) && $search!=""){
	  ...
	  $msg_qry ="SELECT * FROM es_notice_messages WHERE to_id=".$_SESSION['eschools']['user_id']." AND to_type='staff' and to_status!='deleted' ".$condition." ";
...
if($action=="delete"){
	...
	header("location: ?pid=31&action=mails_received&emsg=29");
}
if($action=="deletesent"){
$db->update('es_notice_messages', "from_status ='deleted'", 'es_messagesid =' . $es_messagesid);
header("location: ?pid=31&action=sentmails&emsg=29");
}

if($action=="fullmessage"){
 $msg_qry ="SELECT * FROM es_notice_messages WHERE to_id=".$_SESSION['eschools']['user_id']." AND to_type='staff' and es_messagesid=".$es_messagesid;
...
$condition = "";
	if(isset($search) && $search!=""){
	   ...
	   $msg_qry ="SELECT * FROM es_notice_messages WHERE from_id=".$_SESSION['eschools']['user_id'].$condition." AND from_type='staff' and from_status!='deleted'";
	 $no_records = sqlnumber($msg_qry);
...
if($action=="fullmessage_sent"){
 $msg_qry ="SELECT * FROM es_notice_messages WHERE from_id=".$_SESSION['eschools']['user_id']." AND from_type='staff' and es_messagesid=".$es_messagesid;
 $details_message=$db->getrow($msg_qry);
}
