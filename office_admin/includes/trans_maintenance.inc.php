...
if($action=='add_pickup_point' && isset($_POST['add_pickup_point'])){

	$insert_data = insert_into('transport_pickup_points',array('academic_id' => $_POST['academy_year_id'], 'pickuppoint_name' => $_POST['pickup_point_name'], 'annual_charges' => $_POST['annual_charge'], 'created_on' => date('Y-m-d'), 'ledger_id' => $_POST['ledger_id']));

	header("location:?pid=142&action=transportpickuppoint");
}

if($action=='edit_pickup_point' && isset($_POST['edit_pickup_point'])){

	$update_data = update_where('transport_pickup_points',array('academic_id' => $_POST['academy_year_id'], 'pickuppoint_name' => $_POST['pickup_point_name'], 'annual_charges' => $_POST['annual_charge'], 'created_on' => date('Y-m-d'), 'ledger_id' => $_POST['ledger_id']), array('tr_place_id' => $_POST['tr_place_id']));

	header("location:?pid=142&action=transportpickuppoint");
}

if($action=="delete_pickup_point"){
	$delete = delete_where('transport_pickup_points',array('tr_place_id' => $_GET['pickup_id']));
	header("location:?pid=142&action=transportpickuppoint");
}
